...
layout (location = 1) in vec3 vertlimitnormal_camera_fs;
...
void main() {
  ...
  vec3 lightcolour = vec3(1.0, 0.0, 0.0); //red color
  ...
  vec3 normal;
  normal = normalize(vertlimitnormal_camera_fs);
  
  vec3 surftolight = normalize(lightpos - vertlimitcoords_camera_fs);
  float diffusecoeff = max(0.0, dot(surftolight, normal));
  
  vec3 camerapos = vec3(0.0);
  vec3 surftocamera = normalize(camerapos - vertlimitcoords_camera_fs);
