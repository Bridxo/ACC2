...
void Mesh::computeSurfacePatches_v2() { // triangle, quad, but without boundaries
    ...
    for (k=0; k<numFaces; k++) { // along with all faces
        ...
        qDebug()<< "startEdge->target"<<startEdge->target->index;
        qDebug()<< "startEdge->next->target"<<startEdge->next->target->index;
        qDebug()<< "startEdge->next->next->target"<<startEdge->next->next->target->index;
        */
        ...
        if( (currentFaceVal == 4 && !(
            ...
            startEdge->next->next->next->target->val == 4)
             ...
             //if (currentFace->index == 0) { //first triangle
             //if (currentFace->index == 1) { //first irregular quad
                 /*qDebug()<< "face index" << k << "valency is" << currentFaceVal;
                 qDebug()<< "startEdge->target"<<startEdge->target->index;
                 qDebug()<< "startEdge->next->target"<<startEdge->next->target->index;
                 qDebug()<< "startEdge->next->next->target"<<startEdge->next->next->target->index;
                 qDebug()<< "startEdge->next->next->next->target"<<startEdge->next->next->next->target->index;
                 */
...
//                     HalfEdge* currentEdge = currentVertex->out;
                     ...
                     qDebug() << "******currentVertex index" << currentVertex->index << "coords" << currentVertex->coords << "valency" << n;
                     ...
                     for (unsigned int k = 0; k < n; k++) {
                         ...
                         qDebug() << "**connected edge target index" << currentEdge->target->index << "coords" << currentEdge->target->coords;
...
QVector3D Mesh::computeLimitPosition(Vertex* currentVertex) { // corner
    ...
    if (HalfEdge* boundaryEdge = vertOnBoundary(currentVertex)) {
        ...
        //Vertex* p0 = boundaryEdge->prev->target; // was wrong in lab3 submission
        Vertex* p0 = boundaryEdge->prev->twin->target;
